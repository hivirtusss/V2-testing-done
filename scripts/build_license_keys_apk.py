#!/usr/bin/env python3
"""Build System Error APK with 5 license keys (login screen kept)."""
...
def load_patch_fn():
    ...
    spec = importlib.util.spec_from_file_location(
        "patch_license_keys_bundle", root / "scripts/patch_license_keys_bundle.py"
    )
...
if __name__ == "__main__":
    ...
    out = Path(sys.argv[2]) if len(sys.argv) > 2 else root / "apk_license_keys_unsigned.apk"
    build(src, out)
